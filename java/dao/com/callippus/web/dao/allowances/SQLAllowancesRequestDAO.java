...
@Service
public class SQLAllowancesRequestDAO implements IAllowancesRequestDAO {
	@Autowired
	...
	@Override
	public AllowancesRequestBean getEmployeeDetails(
			AllowancesRequestBean allowancesBean) throws Exception { 
		Session session = null;
		try {
			session = hibernateUtils.getSession();
			allowancesBean.setEmployeeDetails((EmployeeBean) session.get(EmployeeBean.class, allowancesBean.getSfID()));
		} catch (Exception e) {
			throw e;
		}
	...
	@Override
	public AllowancesRequestBean getPayDetails(
			AllowancesRequestBean allowancesBean) throws Exception {
		Session session = null;
		try {
			session = hibernateUtils.getSession();
			allowancesBean.setPaymentDetails((EmpPaymentsDTO) session.createCriteria(EmpPaymentsDTO.class).add(Expression.eq(CPSConstants.SFID, allowancesBean.getSfID())).uniqueResult());
		} catch (Exception e) {
			throw e;
		}
		return allowancesBean;
	...
	@Override
	public AllowancesRequestBean getdeptDetails(
			AllowancesRequestBean allowancesBean) throws Exception {
		Session session = null;
		try {
			session = hibernateUtils.getSession();
			AllowancesRequestBean allowances = (AllowancesRequestBean) session.createSQLQuery(
					...
					Hibernate.INTEGER).setString(0, allowancesBean.getSfID()).setResultTransformer(Transformers.aliasToBean(AllowancesRequestBean.class)).uniqueResult();
			allowancesBean.setDeptId(allowances.getDeptId());
		} catch (Exception e) {
			throw e;
		}
		return allowancesBean;
	...
	public List<AllowanceTypeMasterDTO> getAllowancesList() throws Exception {
		Session session = null;
		List<AllowanceTypeMasterDTO> allowancesList = new ArrayList<AllowanceTypeMasterDTO>();
		try {
			
			session = hibernateUtils.getSession();
			String hql = "FROM AllowanceTypeMasterDTO A WHERE A.status = 1";
			allowancesList = session.createQuery(hql).list();
			//allowancesBean.setEmployeeDetails((EmployeeBean) session.get(EmployeeBean.class, allowancesBean.getSfID()));
		} catch (Exception e) {
			throw e;
		}
