...
<div id="Pagination1">
<%int i=1; %>
	<display:table name="${sessionScope.PromotionPayUpdateList}" excludedParams="*"
		...
		<c:otherwise>
		<display:column title="Pay Status" style="width:7%" sortable="true"> </display:column>
		</c:otherwise>
		</c:choose>	
		...
		<display:column title="Variable Increments" style="width:2%" ><div id="${fixationList.variableIncr}"><fmt:formatNumber value="${fixationList.variableIncr}"></fmt:formatNumber></div></display:column>
		<display:column title="Variable IncrValue" style="width:5%" >
					<input type="text" name="variableIncrValue" id="variableIncrValue<%=i %>" value="${fixationList.varIncrValue}" size="5" onkeypress="return checkFloat(event,'variableIncrValue<%=i %>');" onchange="javascript:clickRadio(this)"/>
		</display:column>        <!--This is new column for Variable increment value  -->
