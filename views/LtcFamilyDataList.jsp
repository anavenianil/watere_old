	...
	<div>
		<div id="Pagination">
			<display:table name="${sessionScope.LtcFamilyMemberList}" excludedParams="*"
				...
				sort="list">
					<display:column  style="width:5%;text-align:center" title='<input type=checkbox  name="selectall" id="selectall" onclick="checkBoxCheckAll(this.name,\'row\'); checkSelfApplying();"/>'>
						<input type="checkbox" class="row" name="row" id="${dataList.id}" onclick="checkBoxCheck(this.name); checkSelfApplying();"/>
					</display:column>
				<%-- 	<display:column  style="width:5%;text-align:center" title='<input type=checkbox  name="selectall" id="selectall" />'>
						<input type="checkbox" class="row" name="row" id="${dataList.id}" onclick="checkBoxCheck(this.name);"/>
					</display:column> --%>
...
<c:if test="${sessionScope.EmployeeDetails.handicap != '0' && not empty sessionScope.EmployeeDetails.handicap}">
		<script>
			$jq('#dataList').append('<tr class="odd"><td>Escort</td><td><input type="text" id="escortMemName" maxlength="50"/></td><td><input type="text" id="escortMemDOB" maxlength="20"/></td><td><input type="text" id="escortMemAge" maxlength="3"/></td><td><input type="text" id="escortMemRelation" maxlength="20"/></td></tr>');
		</script>
</c:if>
	    <script>
			...
			<display:table name="${sessionScope.LtcFamilyMemberList}" excludedParams="*"
				...
				sort="list">
					<display:column  style="width:5%;text-align:center" title='<input type=checkbox  name="selectall" id="selectall" onclick="checkBoxCheckAll(this.name,\'row\')"/>'>
						<input type="checkbox" class="row" name="row" id="${dataList.id}" />
					</display:column>
					<display:column title="Name" style="width:15%">${dataList.name}</display:column>
					<display:column title="DOB" style="width:8%">${dataList.dob}</display:column>
					<display:column style="width:18%;text-align:center" title="Age"> <span><font color='red'>${dataList.age} </font></span> as on ${dataList.toDay} </display:column>
