...
<%int i=0; %>
	<display:table name="${sessionScope.emergencyList}" excludedParams="*"
		...
		<display:column  style="width:3%;vertical-align:middle" title='<input type=checkbox  name="selectall" id="selectall" onclick="checkBoxCheckAll(this.name,\'rowe\');"/>'>
			<input type="checkbox" value="${emergency.requestId}" class="rowe" name="rowe" id="emer<%=i %>" onclick="checkBoxCheck(this.name);"/>
		</display:column>
		...
		<display:column title="Amount" style="width:9%" >&nbsp;${emergency.amountClaimed}</display:column>
	<display:column title="Finance Amount" style="width:9%" >&nbsp;<input type="text" size="10" name="Amount" value ="${emergency.amountClaimed}"  id="AmountEmer<%=i %>" onkeypress="return checkInt(event,'AmountEmer');"/></display:column>
		
		<%-- <display:column title="Sanction No<span class='mandatory'>*</span>" style="width:6%;vertical-align:middle">&nbsp;<input type="text" size="7" value="${emergency.sanctionNo}" name="sanctionNo" id="sanctionNoEmer<%=i %>" onkeypress="return checkInt(event,'sanctionNoEmer');"/></display:column>
		<display:column title="Bill No<span class='mandatory'>*</span>" style="width:6%;vertical-align:middle">&nbsp;<input type="text" size="7" value="${emergency.billNo}" name="billNo" id="billNoEmer<%=i %>" onkeypress="return checkInt(event,'billNoEmer');"/></display:column>--%>
	<%-- 	<display:column title="CFA Sig<span class='mandatory'>*</span>" style="width:12%">&nbsp;
			<select name="repSig" id="repSigEmer<%=i %>" style="width:100px">
	...
	<%-- <display:column title="Acc Officer.<span class='mandatory'>*</span>" style="width:12%;vertical-align:middle">&nbsp;
			<select name="accOfficer" id="accOfficerEmer<%=i %>" style="width:100px">
				<option value="select">Select</option>
	...
	<%-- 	<display:column title="Report" style="width:6%;vertical-align:middle">&nbsp;<a href="javascript:getReport('${emergency.requestId}','emergency');">Report</a></display:column>--%>
		...
		<display:column title="CDA Amount" style="width:8%;vertical-align:middle">&nbsp;<font size="4.5em"><span class="WebRupee" >R</span></font><input type="text" size="10" name="cdaAmount" id="cdaAmountEmer<%=i %>" onkeypress="return checkInt(event,'cdaAmountEmer');"/></display:column>
		<display:column title="Report" style="width:6%;vertical-align:middle">&nbsp;<a href="javascript:getReport('${emergency.requestId}','emergency');">Report</a></display:column>
	...
	<div class="quarter"><input type="text"  name="sanctionNo" id="sanctionNoEmer" onkeypress="javascript:return checkInt(event);"/></div>
	                </div>
	<div class="line">
	<div class="quarter bold">Bill No <span class="mandatory">*</span></div>
	<div class="quarter"><input type="text" name="billNo" id="billNoEmer" onkeypress="javascript:return checkInt(event);"/></div></div>
	<div class="line">
    <div class="quarter bold">Account Officer<span class="mandatory">*</span></div>
    <div class="quarter"><select name="accOfficer" id="accOfficerEmer" style="width:160px"><option value="select">Select</option>
				<c:forEach var="acc" items="${sessionScope.AccountOfficerList}">
					<option value="${acc.sfid}">${acc.name}</option>
				</c:forEach> </select>
				</div>
    ...
    <div class="quarter"><select name="repSig" id="repSigEmer" style="width:160px"><option value="select">Select</option>
				<c:forEach var="cfa" items="${sessionScope.CfaOfficerList}">
					<option value="${cfa.sfid}">${cfa.name}</option>
				</c:forEach> </select>
				</div>
