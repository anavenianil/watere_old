...
function manageTrainingMaster(type, jsonMasterDataList) {
	...
	if (status) {
		...
		if (document.forms[0].typeValue != null)
			$jq('#name').val($jq.trim($jq('#typeValue').val()));
...
function clearCourses(type) {
	...
	if ($jq('#nameCounter').length > 0)
		$jq('#nameCounter').val('500');
	if ($jq('#subjcounter').length > 0)
		$jq('#subjcounter').val('200');
	if ($jq('#subjsubcounter').length > 0)
...
function courseSubmit(type) {
	...
	if (status) {
		...
		var requestDetails = {
				...
				"courseSubjCategory" : $jq.trim($jq("#courseSubjCategory").val()),
				"courseSubjSubCategory" : $jq.trim($jq("#courseSubjSubCategory").val()),
				"fee" : $jq("#fee").val(),
	...
	for ( var i = 0; jsonMasterDataList != null
		...
		if (jsonMasterDataList[i].id == id) {
			...
			if (jsonMasterDataList[i].courseSubjCategory != null)
			{
				...
				$jq('#subjcounter').val(
						200-jsonMasterDataList[i].courseSubjCategory.length);
			}
			else
			{
				$jq('#courseSubjCategory').val('');
			...
			if (jsonMasterDataList[i].courseSubjSubCategory != null)
			{
				...
				$jq('#subjsubcounter').val(200-
						jsonMasterDataList[i].courseSubjSubCategory.length);
			}
			else
			{
				$jq('#courseSubjSubCategory').val('');
			...
			$jq('#nameCounter').val(500-jsonMasterDataList[i].name.length);
...
function courseDurationSubmit(type, jsonMasterDataList, jsonFinancialYearList,year) {
	...
	if ($jq("#endDate").val() == "") {
		...
		//if(($jq("#startDate").val().split("-")[2] && $jq("#endDate").val().split("-")[2] >=year.split("-")[0] && $jq("#startDate").val().split("-")[2] && $jq("#endDate").val().split("-")[2] <=year.split("-")[1]) || ($jq("#startDate").val().split("-")[2] && $jq("#endDate").val().split("-")[2] <=year.split("-")[1] &&  $jq("#startDate").val().split("-")[2] && $jq("#endDate").val().split("-")[2] >=year.split("-")[1])){
		if(($jq("#startDate").val().split("-")[2] >= year.split("-")[0] && $jq("#startDate").val().split("-")[2] <= year.split("-")[1]) && ($jq("#endDate").val().split("-")[2] >= year.split("-")[0] && $jq("#endDate").val().split("-")[2] <= year.split("-")[1])){
		    status =true;
		}else{
			errorMessage+= "Start Date and End Date should be between Course Year  "+year+"";
			status = false;
		}
		if (status) {
			var startDate = $jq("#startDate").val();
...
function hrdgBoardMemberSubmit(type,jsonHRDGBoardMemberTypeList,jsonHRDGBoardMemberList) {
	...
	if (status) {
		...
		var requestDetails = {
			...
			"type" : $jq("#type").val(),
			"id" : $jq("#id").val(),
			"boardMemberSfid" : $jq.trim($jq("#sfid").val()),
...
function trainingCirculationSubmit() {
	...
	if (status) {
		...
		var sDate = $jq.trim(document.getElementById('durationStartDate').innerHTML);//$jq("#durationStartDate").val();
		var circulationDate = $jq.trim($jq("#circulationDate").val());
...
function clearTrainingCirculation() {
	...
	$jq("#brochure").val('');
	$jq("#organizer").val('');
	$jq("#organizerCounter").val('2000');
	}
	else
	{
		$jq("#organizerCounter").val('2000');
		var org = $jq("#organizer").val();
		if(org.length>0)
		{
			$jq("#organizerCounter").val(2000-org.length);
		}
		$jq("#newBrochure").hide();
...
//	}
	...
	if (status) {
		$jq.trim($jq("#currentAssignment").val());
		$jq.trim($jq("#relevance").val());
...
function saveNominations(type) {
	...
	if (type == 'boardSelection') {
		...
		if ($jq("#letterFormatId2").val() == "") {
			errorMessage += "Select Serial Number.\n";
			status = false;
			$jq("#letterFormatId2").focus();
		}
	...
	if (type == 'FinalAlert') {
		...
		if ($jq("#letterFormatId4").val() == "") {
			errorMessage += "Select Serial Number.\n";
			status = false;
			$jq("#letterFormatId4").focus();
		}
...
function trainingRegionSubmit(type)
{
	...
	if (status) {
		var requestDetails = {
				"name" : $jq.trim($jq("#name").val()),
				"id" : $jq("#id").val(),
				"description" : $jq.trim($jq("#description").val()),
				"type" : $jq("#type").val(),
				"trainingTypeId" : $jq("#trainingTypeId").val(),
...
function trainingMasterSubmit(type)
{
	...
	var status = true;
		...
		if ($jq("#trainingRegionId").val() == "") {
			errorMessage += "Select Region Name.\n";
			status = false;
			$jq("#trainingRegionId").focus();
		}
		if ($jq("#typeValue").val() == "") {
			errorMessage += "Enter Training Institute.\n";
			status = false;
			$jq("#typeValue").focus();
		}
	...
	if (status) {
		...
		var requestDetails = {
				...
				"name" : $jq.trim($jq("#name").val()),
				"id" : $jq("#id").val(),
				"shortName" : $jq.trim($jq("#shortName").val()),
...
function trainingVenueMasterSubmit(type,jsonMasterDataList)
{
	...
	if(status)
	{
		var requestDetails = {
				...
				"address" : $jq.trim($jq("#address").val()),
				"phone" : $jq.trim($jq("#phone").val()),
				"fax" : $jq.trim($jq("#fax").val()),
				"email" : $jq.trim($jq("#email").val()),
				"ddAddress" : $jq.trim($jq("#ddAddress").val()),
...
function courseContentSubmit(type)
{
	...
	if(content.length==0){
		...
		else
		{
			...
			del = confirm("Do you want to clear this item?");
		}
	}
	...
	if (status && del) {
		...
		var requestDetails = {
				...
				"categoryId" : $jq("#categoryId").val(),
				"type" : $jq("#type").val(),
				"courseContent" : $jq.trim($jq("#courseContent").val()),
...
function setLastAttendedCourse(jsonLastAttendedCourse)
{
	...
	$jq('#lastAttendedCourseCounter').val(1500);
					$jq('#lastAttendedCourseCounter').val(
							$jq('#lastAttendedCourseCounter').val() - $jq.trim($jq('#lastAttendedCourse').val()).length);
}
...
function manageCourseAttended(type,jsonFinancialList,jsonMasterDataList)
{
	...
	if (status) {
		...
		var requestDetails = {
				...
				"fee" : $jq.trim($jq("#fee").val()),
				"serviceTax" : $jq.trim($jq("#serviceTax").val()).replace('%',''),
				"type" : $jq("#type").val(),
...
function deleteCourseAttended(id,type)
{
	...
	var check = confirm("Do u Want To Delete This Record ?");
	if(check){
		var requestDetails = {
				"id" : id,
				"trainingTypeId" : $jq("#trainingTypeId").val(),
...
function checkBoxCheckAllDisabled(type, rowType,typeId) {
	if($jq('input:checkbox[name='+ type +']').is(':checked'))
		$jq('.' + rowType).attr('checked', 'checked');	
	else
		if(typeId=='nomination'){
			if($jq('#nominations tr:not(:first)').each(function(){
				if(!$jq(this).find('td').eq(0).find('input:checkbox').attr('disabled')){
					$jq(this).find('td').eq(0).find('input:checkbox').removeAttr("checked");
					}
			}));	
		}else if(typeId=='boardSelection'){
			if($jq('#boardSelections tr:not(:first)').each(function(){
				if(!$jq(this).find('td').eq(0).find('input:checkbox').attr('disabled')){
					$jq(this).find('td').eq(0).find('input:checkbox').removeAttr("checked");
					}
				
			}));
		}else if(typeId=='ADSelection'){
			if($jq('#ADSelections tr:not(:first)').each(function(){
				if(!$jq(this).find('td').eq(0).find('input:checkbox').attr('disabled')){
					$jq(this).find('td').eq(0).find('input:checkbox').removeAttr("checked");
					}
				
			}));
		}else if(typeId=='FinalAlert'){
			if($jq('#FinalAlerts tr:not(:first)').each(function(){
				if(!$jq(this).find('td').eq(0).find('input:checkbox').attr('disabled')){
					$jq(this).find('td').eq(0).find('input:checkbox').removeAttr("checked");
					}
