...
// $Id: calendar.js,v 1.3 2005/07/18 08:32:59 clint Exp $
...
Calendar.tableMouseUp = function(ev) {
	...
	if (mon) {
		...
		if (mon.month != date.getMonth()) {
			...
			//cal.callHandler();//changing combo month will not be reflect::::vinod
		}
	} else {
		...
		if (year) {
			...
			if (year.year != date.getFullYear()) {
				...
				cal.setDate(date);
				cal.dateClicked = false;
				//cal.callHandler();//changing combo year will not be reflect::::vinod
			}
		}
	}
	with (Calendar) {
...
Calendar.cellClick = function(el, ev) {
	...
	if (typeof el.navtype == "undefined") {
		...
		if (!date.equalsTo(cal.date)) {
			cal.setDate(date);
			//newdate = true;//changing month and year one by one will not reflect::::vinod
		}
	}
	if (newdate) {
		cal.callHandler();
...
Calendar.prototype._init = function (firstDayOfWeek, date) {
	...
	var holidays = Calendar._TT["HOLIDAYS"];
	for (var i = 0; i < 6; ++i, row = row.nextSibling) {
		...
		for (var j = 0; j < 7; ++j, cell = cell.nextSibling, date.setDate(date.getDate() + 1)) {
			...
			if (!cell.disabled) {
				...
				if (holidays.indexOf(date.getDate()+'-'+getMonthMON(date.getMonth()+1)+'-'+date.getFullYear()) != -1) {
					cell.className += cell.otherMonth ? " oweekend" : " weekend";
				}
			}
		}
